...
type Parser struct {
	...
	curToken  token.Token // 指向当前标记
	peekToken token.Token // 指向下一个标记
...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerPrefix(token.LPAREN, p.parseGroupedExpression) // ( 左括号
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	prefix := p.prefixParseFns[p.curToken.Type] // 检查前缀位置中是否有
	if prefix == nil {
	...
	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		infix := p.infixParseFns[p.peekToken.Type] // 中序表达式
		if infix == nil {
			return leftExp
		}
